...
const baseStyles = `
  ...
  .feature-box { background-color: #ffffff; border: 1px solid #e6ebf1; border-left: 3px solid #5C8DB8; border-radius: 6px; padding: 18px 20px; margin: 24px 0; }
...
const templates: Record<string, (d: TemplateData) => { subject: string; html: string }> = {
  ...
  nudge_day1: (d) => ({
    subject: "[TEST] Set up TrackSpeed for your next sprint session",
    html: wrapTemplate(`<h1>Set Up Your Next Sprint Session</h1><p>Hi ${d.name},</p>
      ...
      <div class="feature-box"><p style="font-weight:600;color:#1a1a1a;margin:0 0 12px;">Next session setup:</p>
      ...
      <p style="font-size:14px;margin:0;">3. Join both phones to the same TrackSpeed session</p></div>
  ...
  checkin_day14: (d) => ({
    subject: "[TEST] Two weeks in - here's how to level up your training",
    html: wrapTemplate(`<h1>Two Weeks In - How's It Going?</h1><p>Hi ${d.name},</p>
      <p>You've logged <strong>${d.sessionCount} sessions</strong> so far. That's real commitment!</p>
      <p>Want to take your training to the next level? Here's what Pro unlocks:</p>
      <div class="feature-box"><p><strong>Multi-device timing</strong> - One phone at start, one at finish for professional accuracy</p>
      <p><strong>Unlimited sessions</strong> - Train as much as you want, no limits</p>
      <p style="margin:0;"><strong>Full history & analytics</strong> - See your progress over weeks and months</p></div>
      <p style="text-align:center;"><a href="#" class="btn">Upgrade to Pro</a></p>
      <p>Keep pushing!<br>The TrackSpeed Team</p>`),
  }),
  first_session: (d) => ({
    subject: "[TEST] Your first TrackSpeed session summary is ready",
    html: wrapTemplate(`<h1>Your First Session Summary</h1><p>Hi ${d.name},</p>
      ...
      <p style="font-size:14px;margin:0 0 8px;"><strong>Distance</strong> - ${d.distance}</p>
      <p style="font-size:14px;margin:0 0 8px;"><strong>Best time</strong> - ${d.bestTime}</p>
      <p style="font-size:14px;margin:0 0 8px;"><strong>Latest run</strong> - ${d.sprintTime}</p>
      <p style="font-size:14px;margin:0 0 8px;"><strong>Average time</strong> - ${d.averageTime}</p>
      <p style="font-size:14px;margin:0;"><strong>Runs recorded</strong> - ${d.runCount}</p></div>
      <p>Use this as your baseline. After your next sprint session, compare the summary to see how your times are moving.</p>
      <p style="text-align:center;"><a href="#" class="btn">Run Another Sprint</a></p>
  ...
  convert_day7: (d) => ({
    ...
    html: wrapTemplate(`<h1>You've run ${d.sessionCount} sessions!</h1><p>Hi ${d.name},</p>
      <p>Great progress! Pro keeps the full training workflow unlocked:</p>
  ...
  winback: (d) => ({
    ...
    html: wrapTemplate(`<h1>We Miss You at the Track</h1><p>Hi ${d.name},</p>
      <p>It's been a while since you used TrackSpeed. We'd love to have you back at the track.</p>
  ...
  feature_update: (d) => ({
    subject: `[TEST] TrackSpeed feature spotlight: ${d.featureTitle}`,
    html: wrapTemplate(`<p style="display:inline-block;background:#5C8DB8;color:#fff;font-size:12px;font-weight:700;padding:4px 12px;border-radius:4px;margin:0 0 16px;">FEATURE SPOTLIGHT</p>
      ...
      <p style="font-size:14px;margin:0;">1. Open TrackSpeed<br>2. Look for ${d.featureTitle} in the app<br>3. Try it during your next session</p></div>

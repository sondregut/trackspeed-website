"use client";

import { useState } from "react";
import { smsCampaigns } from "./SMSCampaignTable";

interface TestSMSFormProps {
  onSend: (template: string, phone: string) => Promise<void>;
}

export default function TestSMSForm({ onSend }: TestSMSFormProps) {
  const [template, setTemplate] = useState(smsCampaigns[0].templateKey);
  const [phone, setPhone] = useState("");
  const [sending, setSending] = useState(false);
  const [message, setMessage] = useState<{
    type: "success" | "error";
    text: string;
  } | null>(null);

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!phone || !template) return;

    setSending(true);
    setMessage(null);

    try {
      await onSend(template, phone);
      setMessage({ type: "success", text: "Test SMS sent successfully!" });
      setPhone("");
    } catch (error) {
      setMessage({
        type: "error",
        text: error instanceof Error ? error.message : "Failed to send SMS",
      });
    } finally {
      setSending(false);
    }
  }

  return (
    <div className="bg-[#2B2E32] rounded-xl border border-[#3D3D3D] p-6">
      <h2 className="text-lg font-semibold text-white mb-1">Send Test SMS</h2>
      <p className="text-sm text-[#9B9A97] mb-4">
        Send a test SMS to verify templates
      </p>

      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label
            htmlFor="sms-template"
            className="block text-sm font-medium text-[#9B9A97] mb-1"
          >
            Template
          </label>
          <select
            id="sms-template"
            value={template}
            onChange={(e) => setTemplate(e.target.value)}
            className="w-full px-3 py-2 bg-[#1A1A1A] border border-[#3D3D3D] rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#5C8DB8]"
          >
            {smsCampaigns.map((campaign) => (
              <option key={campaign.id} value={campaign.templateKey}>
                {campaign.name}
              </option>
            ))}
          </select>
        </div>

        <div>
          <label
            htmlFor="phone"
            className="block text-sm font-medium text-[#9B9A97] mb-1"
          >
            Phone Number
          </label>
          <input
            type="tel"
            id="phone"
            value={phone}
            onChange={(e) => setPhone(e.target.value)}
            placeholder="+1234567890"
            required
            className="w-full px-3 py-2 bg-[#1A1A1A] border border-[#3D3D3D] rounded-lg text-white placeholder-[#787774] focus:outline-none focus:ring-2 focus:ring-[#5C8DB8]"
          />
          <p className="text-xs text-[#787774] mt-1">
            Include country code (e.g., +1 for US)
          </p>
        </div>

        {message && (
          <div
            className={`p-3 rounded-lg text-sm ${
              message.type === "success"
                ? "bg-[#22C55E]/10 text-[#22C55E] border border-[#22C55E]/20"
                : "bg-[#FC0726]/10 text-[#FC0726] border border-[#FC0726]/20"
            }`}
          >
            {message.text}
          </div>
        )}

        <button
          type="submit"
          disabled={sending || !phone}
          className="w-full btn-primary px-4 py-2 rounded-lg font-medium disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {sending ? "Sending..." : "Send Test SMS"}
        </button>
      </form>

      {/* SMS Info */}
      <div className="mt-4 p-3 bg-[#1A1A1A] rounded-lg">
        <p className="text-xs text-[#787774]">
          <strong className="text-[#9B9A97]">Note:</strong> SMS will be sent via
          Twilio. Standard messaging rates may apply for the recipient.
        </p>
      </div>
    </div>
  );
}

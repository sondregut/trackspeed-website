...
export default function SMSSendLog({ logs, loading }: SMSSendLogProps) {
  if (loading) {
    return (
  ...
  return (
    <div className="bg-[#2B2E32] rounded-xl border border-[#3D3D3D] overflow-hidden">
      <div className="p-4 border-b border-[#3D3D3D]">
        <h2 className="text-lg font-semibold text-white">Recent SMS Sends</h2>
        <p className="text-sm text-[#9B9A97] mt-1">Last 50 SMS messages sent</p>
      </div>
          ...
          <tbody>
            {logs.map((log) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-sm text-[#9B9A97] font-mono">
                  {maskPhone(log.phone)}
                </td>
                ...
                <td className="px-4 py-3 text-sm text-[#787774] font-mono">
                  {log.twilio_sid ? log.twilio_sid.slice(0, 12) + "..." : "—"}
                </td>
              </tr>
            ))}
            {logs.length === 0 && (
